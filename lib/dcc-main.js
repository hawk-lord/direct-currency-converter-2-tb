    ...
    const DccUi = function () {
        const showInputPanel = function() {
            ...
            while (xulWindows.hasMoreElements()) {
                ...
                //const inputPanel = this.E('translator-input-panel');
                // [object XULElement]
                if (document.getElementById("dcc-input-panel")) {
                    console.error("dcc-input-panel is already present")
                    break;
                }
                const inputPanel = document.createElementNS(NS_XUL, "panel");
...
*/
                const popupSet = document.getElementById("messagepanebox");
                try {
                    if (popupSet) {
                        popupSet.appendChild(inputPanel);
                    }
                ...
                console.error("inputPanel.id " + inputPanel.id);
                console.error("window.outerWidth " + window.outerWidth);
                const left = (window.outerWidth - 300) / 2;
                const top = (window.outerHeight - 150) / 2;
                console.error("left " + left);
                console.error("top " + top);
                try {
                    // inputPanel.openPopup(null, null, left, top, false, false);
                    inputPanel.openPopup(null, null, 50, 50, false, false);
                }
                catch(err) {
                    console.error(err);
                }
